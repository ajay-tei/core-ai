...
public interface IPromotionOrchestrationService
{
    ...
    /// <paramref name="targetLlmConfigId"/> (Agent promotions only) explicitly sets the promoted row's LlmConfigId in the target environment; when null, the target's own existing LlmConfigId is left untouched (LlmConfigId is deliberately excluded from the portable snapshot, so this is already the default "keep existing" behavior on re-promotion).
    /// <paramref name="excludedLogicalIds"/> skips these specific optional dependents (see <see cref="IPromotionDependencyResolver.GetOptionalDependentsAsync"/>, e.g. an Agent's scheduled tasks) — hard cascade dependencies are never excludable.</summary>
    Task<PromotionResult> PromoteAsync(int tenantId, string objectType, Guid logicalId, int fromEnvironmentId, int toEnvironmentId, string? createdBy, string? changeNote, int? targetLlmConfigId, CancellationToken ct, IReadOnlyList<Guid>? excludedLogicalIds = null);
