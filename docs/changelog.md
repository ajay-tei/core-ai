...
## [2026-08-24] LLM config: multi-API-key rotation per tenant config, plus 2 bugs found while validating it live

**Feature — multi-key rotation**: tenant-level named LLM configs can now hold additional API keys alongside the existing primary `ApiKey`, to spread concurrent load across more than one key's rate-limit budget (reduces 429/504s under load — follow-up to the concurrent-load review). `TenantLlmConfigEntity.AdditionalApiKeysJson` (nullable JSON string array, plaintext, matching the existing `ApiKey` convention — no `ICredentialEncryptor`). `ResolvedLlmConfig` gained `ApiKeys`/`EffectiveApiKeys` (always non-empty, falls back to `[ApiKey]`); `LlmConfigState.Overlay` rebuilds the pool only when the primary key itself is overridden at that level, dedupes, drops blanks, and falls back silently on malformed JSON. Platform/Group levels deliberately do not get a pool (explicit scope decision — their `Overlay` call sites pass `null`). New stateless `ApiKeyPoolSelector` (`PickRandom`/`PickDifferent`, no round-robin cursor or cooldown map in v1). `AnthropicAgentRunner` proactively picks a random pool key at run start when more than one is available, and `CallWithRetryAsync` reactively rotates to a different key (new `IsRateLimitError`, narrower than the existing transient-error check — 502/503/timeouts don't rotate, only 429/`rate_limit`) before the backoff delay on retry. Reused the pre-existing `SetModel(apiKeyOverride:)` mid-run-model-switch mechanism for the rotation callback instead of adding a new method. Admin UI: `LlmForm` (`PlatformLlmConfig.tsx`) gained an optional repeatable "Additional API Keys" editor, only rendered for the tenant-level call site in `TenantDetail.tsx`; backend only ever returns a count (`additionalApiKeyCount`), never raw values, matching the existing masked-key convention.
(`src/Diva.Infrastructure/Data/Entities/LlmConfigEntities.cs`, `src/Diva.Infrastructure/LiteLLM/ILlmConfigResolver.cs`, `src/Diva.Infrastructure/LiteLLM/LlmConfigResolver.cs`, `src/Diva.Infrastructure/LiteLLM/ApiKeyPoolSelector.cs`, `src/Diva.Infrastructure/LiteLLM/AnthropicAgentRunner.cs`, `src/Diva.TenantAdmin/Services/ITenantGroupService.cs`, `src/Diva.TenantAdmin/Services/TenantGroupService.cs`, `src/Diva.Host/Controllers/LlmConfigController.cs`, `admin-portal/src/api.ts`, `admin-portal/src/components/PlatformLlmConfig.tsx`, `admin-portal/src/components/TenantDetail.tsx`; EF migrations `AddAdditionalApiKeysToTenantLlmConfig` for both SQLite and SQL Server)

**Bug found while validating live — zero observability**: the proactive pool pick and reactive rotation had no logging at all, so there was no way to see the feature working even when it was. Live logs for a real agent showed detailed per-iteration/tool-call/credential logging but nothing about the key pool. Fixed: `LogInformation` on proactive pick (`"{PoolSize} keys available, using key ****{Tail}"`) and added the same masked tail to the existing reactive-rotation warning log, matching the established `key ****XXXX` convention already used by `CredentialResolver`/`McpConnectionManager`/`McpCredentialSelector`.

**Bug found while validating live — `InvalidateForTenant` was dead code for named configs**: the cache key format is `llm_resolved_{tenantId}_{configId}_{environmentId}_{modelId}`, but `InvalidateForTenant` only ever cleared the no-config-id combo — any named-config resolution (i.e. every agent with `LlmConfigId` set) kept serving its pre-edit value for up to the full 2-minute cache TTL after an admin edit. Fixed with a real per-tenant version-token pattern: `ResolveAsync` reads/appends a cached version token into its cache key; `InvalidateForTenant` bumps the token, making every previously-cached entry for that tenant unreachable at once. Same bug class as the previously-documented `ICredentialResolver`/MCP-client-cache staleness issues (a bare TTL is not a substitute for real invalidation) — third confirmed instance of it in this codebase. Rewrote the existing `InvalidateForTenant_AllowsFreshResolution` test, which had been asserting the old buggy behavior by manually seeding the stale cache key rather than exercising `ResolveAsync` twice, and added a new test covering the exact real-world scenario (add rotation keys to a named tenant config, invalidate, re-resolve, see the new pool).

**Verification**: `dotnet build Diva.slnx` 0 errors. New tests: 4 in `LlmConfigResolverTests.cs` (pool merge, no-pool fallback, malformed-JSON fallback, Group configs never get a pool) plus the invalidation rewrite/addition (19/19 in that file, full `Diva.TenantAdmin.Tests` 353/353); 5 in new `ApiKeyPoolSelectorTests.cs`. Live-verified against the real deployment: logs show genuine rotation across both configured keys on separate requests, with a real token-usage log line confirming a successfully authenticated call immediately after a rotated key was picked.

---

## [2026-08-24] Anthropic prompt caching: review, hardening, and per-agent 1-hour TTL

**Review**: audited the existing BP1 (static system block) / BP3 (history boundary) / BP4 (sliding tool-result) `cache_control` breakpoint strategy in `AnthropicProviderStrategy.cs` against Anthropic's own docs, and verified empirically against the real trace DB (`data/sessions-trace.db`): 678 recorded Anthropic turns showed a 69% overall cache-hit rate, confirmed cross-session reuse of BP1 for the same agent, and 100% cache-hit rate (19/19) on turn-2+ continuations. Confirmed the "BP2 (tool definitions) not supported" code comments are technically accurate for `Tool` itself but miss that `Tool.Function.CacheControl` exists in SDK 5.10.0 — not used, since the implicit tools-precede-system prefix inclusion already covers it; noted as a possible future enhancement.

**Fix — tool ordering instability**: `SetTools`/`AddExtraTools` now sort tools by name (`StringComparer.Ordinal`) before building the array sent to Anthropic. The tools array is part of the same ordered cache prefix as BP1/BP2 (`tools → system → messages`); `McpConnectionManager.BuildToolDataAsync` iterates a `Dictionary` plus whatever order external MCP servers return their own tool list in, so without a stable sort, a harmless reordering across requests could silently invalidate the entire prompt cache.
(`src/Diva.Infrastructure/LiteLLM/AnthropicProviderStrategy.cs`)

**Feature — optional 1-hour cache TTL**: added `EnableOneHourCache`, applied to BP1 and BP3 (the two long-lived breakpoints most exposed to user think-time and slower iterations) via a new `AnthropicProviderStrategy.BuildLongLivedCacheControl()` helper; BP4 always keeps the plain 5-minute default since it slides every tool exchange. Threaded through `ModelSwitchParameters`/`ModelSwitchCoordinator` so a mid-run model switch preserves the chosen TTL. Shipped first as a global-only `AgentOptions` setting, then made fully per-agent overridable (mirroring the existing `EnableHistoryCaching` pattern exactly): nullable override on `AgentDefinitionEntity` and `GroupAgentTemplateEntity`, wired through `AgentExportService` (export + import), `GroupAgentOverlayMerger`, `TenantGroupService`, `AgentsController`/`ConfigController`/`GroupsController`, and an `AgentBuilder.tsx` toggle (disabled when the effective `EnableHistoryCaching` is off). EF migrations added for both SQLite and SQL Server (`AddEnableOneHourCacheToAgents`). Verified the new field flows correctly through agent environment promotion (`AgentSnapshotSerializer`'s generic JSON snapshot/content-hash mechanism needs no promotion-layer changes once a field is in both `AgentExportService` mapping directions) with a dedicated round-trip test.
(`src/Diva.Core/Configuration/AgentOptions.cs`, `src/Diva.Core/Models/AgentExport.cs`, `src/Diva.Infrastructure/Data/Entities/AgentDefinitionEntity.cs`, `src/Diva.Infrastructure/Data/Entities/TenantGroupEntities.cs`, `src/Diva.Infrastructure/AgentExport/AgentExportService.cs`, `src/Diva.Infrastructure/Groups/GroupAgentOverlayMerger.cs`, `src/Diva.Infrastructure/LiteLLM/AnthropicAgentRunner.cs`, `src/Diva.Infrastructure/LiteLLM/ModelSwitchCoordinator.cs`, `src/Diva.TenantAdmin/Services/ITenantGroupService.cs`, `src/Diva.TenantAdmin/Services/TenantGroupService.cs`, `src/Diva.Host/Controllers/AgentsController.cs`, `src/Diva.Host/Controllers/ConfigController.cs`, `src/Diva.Host/Controllers/GroupsController.cs`, `admin-portal/src/api.ts`, `admin-portal/src/components/AgentBuilder.tsx`)

**Verification**: `dotnet build Diva.slnx` 0 errors. New/updated tests: `AnthropicProviderStrategyCacheTests.cs` (+5: tool-name sorting × 2, TTL on/off × 2, BP4 stays 5-minute), `AgentSnapshotSerializerTests.SerializeThenMaterialize_RoundTrip_PreservesEnableOneHourCache` (new promotion round-trip test, also asserts the content hash is stable across unchanged re-serialization). Full `Diva.Agents.Tests` 376/377 (1 pre-existing documented flaky test, unrelated) and `Diva.TenantAdmin.Tests` 348/348 — no regressions.
